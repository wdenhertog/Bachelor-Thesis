...
class element:
    ...
    def calcsize(self):
        ...
        p = (d1 + d2 + d3) / 2
        return np.sqrt(
            abs(p * (p - d1) * (p - d2) * (p - d3)))  # the absolute value is to suppress faulty negative values
...
class mesh:
    ...
    def setuppdistproperties(self):
        ...
        for i in range(len(pdistarray)):
            pdistarray[i] = [p for p in pdistarray[i] if
                             p.nbzerodist == i]  # filters out the points that ended up in other distance arrays in pdistarray
        return pdistarray
            ...
            if e1.iszeroedge and e1.isboundary and not e1.islevelset:
                ...
                if dangle < np.pi / 4:
                    if p1.zdist < p2.zdist and (not p1.iscorner):
                        ...
                        print(
                            'Error -- no suitable point found to fit to levelsetline due to freedom of movement restrictions on corner points.')
                    boundaryplist.append(plist)
