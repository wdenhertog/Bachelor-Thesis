            ...
            if nbars % 2 == 0:
                ...
                for q,qdata in enumerate(qualityobjects):
                    ...
                    if fun in qdata.fnames:
                        ...
                        plist.append( pyplot.bar(f+(q+1-nbars/2)*width, qdata.after[findex][m][1], width, color=colors[q+1], align='edge') )
            else:
                ...
                for q,qdata in enumerate(qualityobjects):
                    ...
                    if fun in qdata.fnames:
                        findex = qdata.fnames.index(fun)
                        plist.append( pyplot.bar(f+(q+1-nbars//2)*width, qdata.after[findex][m][1], width, color=colors[q+1], align='center') )
...
def simplesol(z,thresh,qdata,relaxthresh, newtriang = False, flip = False, distplot = False, simplepointrelax = False, simplegridtozeropoint = False, redistribute = False):
    ...
    if shortestpath:                           # facitates the shortest path method with some additional plots
        ...
        savename.replace(" ","_")
        print('---Plotting 1')
        m.plotzeroz(directory + 'After_setuppoints_'+ mname + '_' + z.__name__ + '_adjmesh.png', dist = distplot, plotchosenboundary = True)
...
def simplemeshfit(thresh,methodname,hstep,relaxthresh, farr = [cos_times_sin,linear,circle,star,dumbbell],
                  newtriang = False, flip = False, distplot = False, simplepointrelax = False, simplegridtozeropoint = False, redistribute = False):
    ...
    for i in range(len(farr)):
        ...
        simplesol(farr[i],thresh,qdata,relaxthresh, newtriang = newtriang, flip = flip, distplot = distplot,
                  simplepointrelax = simplepointrelax, simplegridtozeropoint = simplegridtozeropoint, redistribute = redistribute)
    qdata.save(hstep)
    return
...
allmethodnames = ['Cut method','Cut and flip method','Shortest path fit','Shortest path fit with redistribution','Shortest path fit with Euler forward relaxation']
